...
class skylineCF(ExplainerBase):
    ...
    def generate_counterfactuals(self, query_instance, features_to_vary, semi_features = ["NumTradesOpeninLast12M", "MSinceMostRecentInqexcl7days", "NumInqLast6M"], target = 0.7):
        ...
        indices_semi_features = [self.data_interface.feature_names.index(i) for i in semi_features]
        ...
        anchor_set = self.find_anchor_set(query_instance, indices_immutable, indices_semi_features, positive_sample, self.model_interface, self.anomaly, target)
        if len(anchor_set) == 0:
            return None, None
    ...
    def find_anchor_set(self, instance, immutable_index, indices_semi_features, positive_sample, clf, anomaly, target = 0.7):
        ...
        indicator_score = (positive_sample[:, indices_semi_features] >= instance[:, indices_semi_features]).sum(1)
        pred3 = (indicator_score == 3)
