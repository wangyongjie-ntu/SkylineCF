...
class GrowingSphere(ExplainerBase):
    ...
    def find_sparse_enemy(self, enemy, target, query_instance):
        ...
        while True:
            ...
            if self.model_interface.predict_ndarray(enemy_prime[np.newaxis, :])[1] > target:
                non_zero_indices.remove(non_zero_indices[argmin])
            else:
                enemy_prime[non_zero_indices[argmin]] = enemy[non_zero_indices[argmin]]
                break
