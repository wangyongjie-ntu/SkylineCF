...
def train_bank_market(filename, epoches, save_name):

    df, df2 = load_bank_market(filename)
    continuous_features = ['age', 'campaign', 'pdays', 'previous', 'emp.var.rate', 'cons.price.idx', 'cons.conf.idx', 'euribor3m', 'nr.employed']

    d = Dataset(dataframe = df, continuous_features = continuous_features, outcome_name = 'y', scaler = MinMaxScaler())
...
if __name__ == "__main__":
    epoch = 100
    saved_name = "weights/bank_market.pth"
    filename = "data/Bank_Market/bank-additional/bank-additional-full.csv"
    train_bank_market(filename, epoch, saved_name)
